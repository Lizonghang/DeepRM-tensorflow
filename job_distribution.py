...
def generate_sequence_work(params):
    ...
    nw_size_seq = np.zeros((simu_len, params.num_res), dtype=int)

    for i in range(simu_len):
        if np.random.rand() < params.new_job_rate:  # a new job comes
            nw_len_seq[i], nw_size_seq[i, :] = nw_dist()

    nw_len_seq = np.reshape(nw_len_seq, [params.num_ex, params.simu_len])
    nw_size_seq = np.reshape(nw_size_seq, [params.num_ex, params.simu_len, params.num_res])
...
class Dist:
    ...
    def bi_model_dist(self):
        ...
        if np.random.rand() < self.job_small_chance:  # small job
            nw_len = np.random.randint(self.job_len_small_lower, self.job_len_small_upper + 1)
        else:   # big job
            nw_len = np.random.randint(self.job_len_big_lower, self.job_len_big_upper + 1)
        ...
        for i in range(self.num_res):
            if i == dominant_res:
                nw_size[i] = np.random.randint(self.dominant_res_lower, self.dominant_res_upper + 1)
            else:
                nw_size[i] = np.random.randint(self.other_res_lower, self.other_res_upper + 1)
